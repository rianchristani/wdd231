...
function displayFutureWeather(data) {
    ...
    let dayAfterTomorrow = document.createElement('p');

    today.innerHTML = `Today: <strong>${data.list[0].main.temp.toFixed(0)}&degC </strong>`
    tomorrow.innerHTML = `${weekDay(data.list[8].dt_txt)}: <strong>${data.list[8].main.temp.toFixed(0)}&degC </strong>`
    dayAfterTomorrow.innerHTML = `${weekDay(data.list[16].dt_txt)}: <strong>${data.list[16].main.temp.toFixed(0)}&degC </strong>`

    div.appendChild(today);
    div.appendChild(tomorrow);
    div.appendChild(dayAfterTomorrow);
